...
namespace Test_Taste_Console_Application.Domain.Services
{
    ...
    public class ScreenOutputService : IOutputService
    {
        ...
        public ScreenOutputService(IPlanetService planetService, IMoonService moonService, ITemperatureService temperatureService)
        {
        ...
        public void OutputAllPlanetsAndTheirMoonsToConsole()
        {
            ...
            for (int i = 0, j = 1; i < planets.Length; i++, j++)
            {
                ...
                var moonCount = planets[i].Moons?.Count ?? 0;
        ...
        public void OutputAllPlanetsAndTheirAverageMoonGravityToConsole()
        {
            ...
            {
                var avgTemp = _temperatureService.CalculateAverageMoonTemperature(planet.Moons);
                Console.WriteLine($"Average Moon Temperature for {CultureInfoUtility.TextInfo.ToTitleCase(planet.Id)}: " +
                              (avgTemp.HasValue ? $"{avgTemp.Value:F2}°C" : "No temperature data available."));
            }
        ...
        public void CalculateAverageMoonTemperature(List<MoonDto> moons)
        {
            if (moons == null || moons.Count == 0)
            {
                Console.WriteLine("No moon data provided.");
                return;
            }
            ...
            Console.WriteLine($"Average Moon Temperature: {average:F2}°C");
        }
